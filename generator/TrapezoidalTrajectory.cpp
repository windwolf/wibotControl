...
namespace control {
bool TrapezoidalTrajectory::plan(float start_pos, float end_pos, float start_vel, float max_vel,
    ...
    if (s * dX < s * dXmin) {
        ...
        vel_rated_   = s * std::sqrt(std::max((dec_rated_ * start_vel * start_vel +
                                             2 * acc_rated_ * dec_rated_ * dX) /
                                                  (dec_rated_ - acc_rated_),
                                              0.0f));
