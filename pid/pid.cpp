...
float PidController::update_parallel(float setpoint, float measurement)
{
    ...
    if (config.intergrator_limit_enable)
    {
        if (_integrator > config.intergrator_limit_max)
        {
            _integrator = config.intergrator_limit_max;
        }
        else if (_integrator < config.intergrator_limit_min)
        {
            _integrator = config.intergrator_limit_min;
        }
    }
